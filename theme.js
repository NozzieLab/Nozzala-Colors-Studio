(function(root,factory){
  ...
  function start(){
    ...
    function boot(){
      ...
      let closing=false,stopping=false,connecting=false,switching=false,revision=0,view='colors',game=null,lastGamePhase=null;
      ...
      refreshDevice=()=>{
        ...
        for(const id of ['egg-colors-tab','egg-game-tab'])doc.getElementById(id).disabled=closing||stopping||switching;
        updateGameControls();
        if(!online){if(game){game.held.fill(false);if(game.active)game.stop('disconnected');}message(deviceBridge?(view==='game'?'连接键盘后开始游戏。':'连接键盘，让这些颜色亮起来。'):'返回工作室连接键盘，即可同步灯光。');}
      };
      const queue=new LightQueue(async task=>{
        if(!deviceBridge?.connected)throw new Error('设备已断开');
        if(task.off)await deviceBridge.off();else if(Object.hasOwn(task,'mole'))await deviceBridge.mole(task.mole);else await deviceBridge.colors(task.colors);
      });
      ...
      deviceKey=({key,pressed})=>game.key(key,pressed);
      ...
      async function mode(next){
        ...
        try{
          if(game.active||game.phase==='finishing')await game.stop('cancelled');
          if(deviceBridge?.connected)await stopLights();
          view=next;egg.scrollTop=0;
          doc.getElementById('egg-colors-panel').hidden=view!=='colors';doc.getElementById('mole-panel').hidden=view!=='game';
          doc.querySelector('.egg-page').classList.toggle('game-view',view==='game');
          doc.getElementById('egg-colors-tab').setAttribute('aria-pressed',String(view==='colors'));doc.getElementById('egg-game-tab').setAttribute('aria-pressed',String(view==='game'));
          doc.getElementById('egg-title').textContent=view==='game'?'打地鼠':'你找到了。';doc.getElementById('egg-subtitle').textContent=view==='game'?'亮哪一键，就打哪一键。':'给灯光一点玩心。';
          if(view==='colors')syncColors();else message('选择难度，开始 60 秒挑战。');
        }catch(error){message(error.message,true);}finally{switching=false;refreshDevice();}
      }
      doc.getElementById('egg-colors-tab').addEventListener('click',()=>mode('colors'));doc.getElementById('egg-game-tab').addEventListener('click',()=>mode('game'));
      doc.addEventListener('visibilitychange',()=>{if(doc.hidden&&game.active)game.stop('hidden');});
      function syncColors(){
      ...
      async function leave(){
        ...
        try{if(game.active||game.phase==='finishing')await game.stop('cancelled');if(deviceBridge?.connected)await stopLights();}catch(error){message(error.message,true);}
      ...
      connect.addEventListener('click',async()=>{connecting=true;refreshDevice();let failure;try{await deviceBridge.connect();if(view==='colors')syncColors();else if(deviceBridge.connected)message('选择难度，开始 60 秒挑战。');}catch(error){failure=error;}finally{connecting=false;refreshDevice();}if(failure)message(failure.message,true);});
      off.addEventListener('click',()=>{if(game.active||game.phase==='finishing')game.stop('cancelled');else stopLights().catch(error=>message(error.message,true));});
      ...
      doc.querySelectorAll('[data-theme-toggle]').forEach(button=>{
        const cancel=bindToggle(button,toggle,()=>{if(!egg||egg.open)return;opener=button;doc.documentElement.classList.add('egg-open');egg.showModal();egg.scrollTop=0;refreshDevice();if(view==='colors')syncColors();else if(deviceBridge?.connected)message('选择难度，开始 60 秒挑战。');});
