(function(root,factory){
  ...
  function start(){
    ...
    function boot(){
      ...
      let closing=false,stopping=false,connecting=false,switching=false,revision=0,view='colors',game=null,rhythm=null,lastGamePhase=null;
      ...
      refreshDevice=()=>{
        ...
        for(const id of ['egg-colors-tab','egg-game-tab','egg-rhythm-tab'])doc.getElementById(id).disabled=closing||stopping||switching;
        updateGameControls();rhythm?.refresh();
        if(!online){rhythm?.disconnect();if(game){game.held.fill(false);if(game.active)game.stop('disconnected');}message(deviceBridge?(view!=='colors'?'连接键盘后开始游戏。':'连接键盘，让这些颜色亮起来。'):'返回工作室连接键盘，即可同步灯光。');}
      };
      const queue=new LightQueue(async task=>{
        if(!deviceBridge?.connected)throw new Error('设备已断开');
        if(task.off)await deviceBridge.off();else if(Object.hasOwn(task,'mole'))await deviceBridge.mole(task.mole);else if(Object.hasOwn(task,'rhythm'))await deviceBridge.rhythm(task.rhythm);else await deviceBridge.colors(task.colors);
      });
      ...
      async function mode(next){
        ...
        try{
          ...
          doc.getElementById('egg-title').textContent=view==='game'?'打地鼠':view==='rhythm'?'六键音游':'你找到了。';doc.getElementById('egg-subtitle').textContent=view==='game'?'亮哪一键，就打哪一键。':view==='rhythm'?'跟着音乐，让每一下都踩准节拍。':'给灯光一点玩心。';
          if(view==='colors')syncColors();else message(view==='rhythm'?'选择曲目，按判定线击打。':'选择难度，开始 60 秒挑战。');
      ...
      doc.addEventListener('visibilitychange',()=>{if(doc.hidden){if(game.active)game.stop('hidden');if(rhythm.active)rhythm.stop('hidden');}});
      function syncColors(){
      ...
      async function leave(){
        ...
        try{if(game.active||game.phase==='finishing')await game.stop('cancelled');if(rhythm.active||rhythm.phase==='finishing')await rhythm.stop('cancelled');if(deviceBridge?.connected)await stopLights();}catch(error){message(error.message,true);}
      ...
      off.addEventListener('click',()=>{if(game.active||game.phase==='finishing')game.stop('cancelled');else if(rhythm.active||rhythm.phase==='finishing')rhythm.stop('cancelled');else stopLights().catch(error=>message(error.message,true));});
      ...
      doc.querySelectorAll('[data-theme-toggle]').forEach(button=>{
        const cancel=bindToggle(button,toggle,()=>{if(!egg||egg.open)return;opener=button;doc.documentElement.classList.add('egg-open');egg.showModal();egg.scrollTop=0;refreshDevice();if(view==='colors')syncColors();else if(deviceBridge?.connected)message(view==='rhythm'?'选择曲目，按判定线击打。':'选择难度，开始 60 秒挑战。');});
